        ...
        if id_list != None and len(id_list) != 0:
            for vm_c in id_list: # Notice: vm_c is uuid
                if self._vm_list.has_key(vm_c):
                    # Update
		    self._vm_list[vm_c]['ins_uuid'] = id_list[vm_c]['ins_uuid']
...
#                        # Alarm threshold 5
#                        if self._vm_list[vm_c]['repeat_num'] <= self.wait:
#                            self._report[self._vm_list[vm_c]['ins_hostname']]['vm_list'].append({'vm':vm_c, 'status':'WAITING - ' + str(self._vm_list[vm_c]['repeat_num']), 'status-ori':self._vm_list[vm_c]['ins_status']})
#                        elif self._vm_list[vm_c]['repeat_num'] > self.wait and self._vm_list[vm_c]['repeat_num'] <= self.reboot:
#                            self._report[self._vm_list[vm_c]['ins_hostname']]['vm_list'].append({'vm':vm_c, 'status':'FAILED - ' + str(self._vm_list[vm_c]['repeat_num']), 'status-ori':self._vm_list[vm_c]['ins_status']})        
#                        else:
#                            self._report[self._vm_list[vm_c]['ins_hostname']]['vm_list'].append({'vm':vm_c, 'status':'REBOOT', 'status-ori':self._vm_list[vm_c]['ins_status']})
...
#		    print self._report[tmp_dict['ins_hostname']]
